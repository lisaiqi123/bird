...
router.get("/huoqu",function (req,res) {
    Weibo.find({},null,{limit:5,sort:{"_id":-1}},function (err,data) {
        if(err){
            console.log(err);
        }else{
            res.send(data);
...
router.get("/add",function (req,res) {
    ...
    Weibo.create({
        ...
        if(err){
            ...
            Weibo.find({time:time},null,{limit:1},function (err,data) {
                if(err){
...
router.get("/fenye",function (req,res) {
    ...
    Weibo.find({},null,{sort:{"_id":-1},skip:start*5,limit:5},function (err,data) {
        if(err){
            console.log(err);
        }else{
            res.send(data);
    ...
    if(index != indexMax){
        Weibo.remove({
            ...
            if(err){
                ...
                Weibo.find({},null,{sort:{"_id":-1},skip:5*index-1,limit:1},function (err,data) {
                    if(err){
                        console.log(err);
                    }else{
                        console.log(data);
